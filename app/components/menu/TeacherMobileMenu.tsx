...
export const TeacherMobileMenu = () => {
                    ...
                    return (
                        ...
                        >
                            ...
                            <div className={`relative px-2 pt-2 rounded-xl transition-all duration-300 ease-out ${isActive ? 'bg-[var(--mainColor)]/10 translate-y-[-4px]' : 'hover:bg-gray-100'}`}>
                                <i className={`pi ${item.icon} transition-all duration-300 ${isActive ? 'text-[var(--mainColor)] scale-110' : 'text-gray-500 group-hover:text-gray-700'}`} style={{ fontSize: '1rem' }} />
                            </div>

                            {/* Текст */}
                            <span className={`text-[0.625rem] font-medium transition-all duration-300 ${isActive ? 'text-[var(--mainColor)] font-bold translate-y-[-2px]' : 'text-gray-500 group-hover:text-gray-700'}`}>{item.label}</span>
